...
func (h *Handler) handleUpdate(req admission.Request, etcd *druidv1alpha1.Etcd, objectMeta metav1.ObjectMeta) admission.Response {
	...
	if etcd.IsReconciliationInProgress() {
		...
		// if resource is in orphan deletion during the etcd reconciliation, the k8s garbage collector
		// will need to make an UPDATE call to the resource in order to manipulate the `orphan` finalizer
