...
func (d *dataVolumesReady) Check(ctx context.Context, etcd druidv1alpha1.Etcd) Result {
	...
	if err != nil {
		...
		res.message = fmt.Sprintf("Unable to fetch warning events for PVCs used by StatefulSet %s: %s", sts.Name, err.Error())
		return res
	}
	...
	res.message = fmt.Sprintf("No warning events found for PVCs used by StatefulSet %s", sts.Name)
